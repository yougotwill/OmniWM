...
final class OmniWorkspaceRuntimeAdapter {
    ...
    @MainActor
    func importSettings(_ settings: SettingsStore) -> Bool {
        ...
        for (workspaceName, descriptions) in settings.workspaceToMonitorAssignments() {
    ...
    func exportState() -> StateExport? {
        ...
        let rc = withUnsafeMutablePointer(to: &rawExport) { exportPtr in
            omni_workspace_runtime_export_state(runtime, exportPtr)
        }
        guard rc == Int32(OMNI_OK) else { return nil }

        let monitorRecords = Self.copyArray(rawExport.monitors, count: rawExport.monitor_count).map { raw in
            StateExport.MonitorRecord(
                ...
                activeWorkspaceId: raw.has_active_workspace_id == 0 ? nil : Self.uuid(from: raw.active_workspace_id),
                previousWorkspaceId: raw.has_previous_workspace_id == 0 ? nil : Self.uuid(from: raw.previous_workspace_id)
            )
        }

        let workspaceRecords = Self.copyArray(rawExport.workspaces, count: rawExport.workspace_count).map { raw in
            StateExport.WorkspaceRecord(
                workspaceId: Self.uuid(from: raw.workspace_id),
                name: Self.string(from: raw.name),
        ...
        let windowRecords = Self.copyArray(rawExport.windows, count: rawExport.window_count).map { raw in
            ...
            return StateExport.WindowRecord(
                handleId: Self.uuid(from: raw.handle_id),
                pid: pid_t(raw.pid),
                windowId: Int(raw.window_id),
                workspaceId: Self.uuid(from: raw.workspace_id),
                hiddenState: hiddenState,
                layoutReason: Self.layoutReason(from: raw.layout_reason)
            )
        }
        ...
        return StateExport(
            ...
            activeMonitorDisplayId: rawExport.has_active_monitor_display_id == 0 ? nil : rawExport.active_monitor_display_id,
    ...
    func workspaceId(forName name: String, createIfMissing: Bool) -> WorkspaceDescriptor.ID? {
        ...
        let rc = withUnsafeMutablePointer(to: &hasId) { hasPtr in
            withUnsafeMutablePointer(to: &rawId) { idPtr in
                omni_workspace_runtime_workspace_id_by_name(
                    runtime,
                    rawName,
                    createIfMissing ? 1 : 0,
                    hasPtr,
    ...
    func summonWorkspace(named name: String, to monitorDisplayId: UInt32) -> WorkspaceDescriptor.ID? {
        ...
        let rc = withUnsafeMutablePointer(to: &hasId) { hasPtr in
            withUnsafeMutablePointer(to: &rawId) { idPtr in
                omni_workspace_runtime_summon_workspace_by_name(
                    runtime,
                    rawName,
                    monitorDisplayId,
                    hasPtr,
    ...
    private static func string(from rawName: OmniWorkspaceRuntimeName) -> String {
        let bytes: [UInt8] = withUnsafeBytes(of: rawName.bytes) { rawBuffer in
            Array(rawBuffer.prefix(Int(rawName.length)))
        }
