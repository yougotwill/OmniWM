...
final class MouseEventHandler {
    ...
    private func applyMouseViewportScrollDelta(
        ...
        var viewportState = controller.workspaceManager.niriViewportState(for: wsId)

        _ = controller.syncZigNiriWorkspace(workspaceId: wsId, selectedNodeId: viewportState.selectedNodeId)
        ...
        if let steps = engine.updateViewportGesture(
            ...
            guard let focusCommand = HotkeyCommand.focus(stepDirection).controllerCommand else { return }

            for _ in 0 ..< abs(steps) {
                guard controller.submitControllerCommand(focusCommand) else { break }
                let selectedNodeId = controller.latestControllerSnapshot?
                    .workspaces
                    .first(where: { $0.workspaceId == wsId })?
                    .selectedNodeId
                viewportState.selectedNodeId = selectedNodeId.map(NodeId.init(uuid:))
            }
        }

        controller.workspaceManager.updateNiriViewportState(viewportState, for: wsId)
    }
