...
final class AXEventHandler: CGSEventDelegate {
    ...
    func handleRemoved(pid: pid_t, winId: Int) {
        ...
        if let entry,
           ...
           controller.effectiveLayoutType(forWorkspaceId: wsId) != .dwindle
        {
    ...
    func handleAppActivation(pid: pid_t) {
        guard let controller else { return }
        guard controller.isCoreRuntimeStarted else { return }
        ...
        if let entry = controller.workspaceManager.entry(forPid: pid, windowId: winId) {
            if controller.submitControllerCommand(.focusWindow(handleId: entry.handle.id)) {
