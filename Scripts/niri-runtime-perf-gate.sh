...
if [[ ! -f "${BASELINE_REPORT}" ]]; then
    ...
    echo "Create it with Scripts/niri-runtime-benchmark.sh --update-baseline" >&2
    exit 1
fi

"${REPO_ROOT}/Scripts/niri-runtime-benchmark.sh"
...
echo "==> Comparing runtime p95/p99 against baseline"
...
let metricNames = baseline.metrics.keys.sorted()
var failures: [String] = []

for metricName in metricNames {
    guard let baselineStats = baseline.metrics[metricName] else { continue }
    guard let latestStats = latest.metrics[metricName] else {
        failures.append("\(metricName): missing in latest report")
        continue
    }
    ...
    if !p95OK || !p99OK {
        failures.append(metricName)
    }
}

if !failures.isEmpty {
    fputs("Runtime performance gate failed for: \(failures.joined(separator: ", "))\n", stderr)
    exit(1)
}

print("Runtime performance gate passed (no p95/p99 regressions).")
SWIFT
